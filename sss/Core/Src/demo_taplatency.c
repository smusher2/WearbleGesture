...
int main(void)
{
    ...
    /* timestamp + print throttle state */
    ...
    while (1)
    {
        ...
        float ax_m = (ax_raw * G_TO_MPS2) / ACC_LSB_PER_G;
        float ay_m = (ay_raw * G_TO_MPS2) / ACC_LSB_PER_G;
        float az_m = (az_raw * G_TO_MPS2) / ACC_LSB_PER_G;
        float gx_m = gx_raw / GYR_LSB_PER_DPS;
        float gy_m = gy_raw / GYR_LSB_PER_DPS;
        float gz_m = gz_raw / GYR_LSB_PER_DPS;
        ...
        */
        ...
        /* ---- Periodic live-data print with timestamp (every 50 ms) ---- */
        if (ts_ms >= next_print_ms) {
            char rawline[192];
            int rn = snprintf(rawline, sizeof(rawline),
                              "[%lu.%03lu] "
                              "AX: %.3f, AY: %.3f, AZ: %.3f, "
                              "GX: %.3f, GY: %.3f, GZ: %.3f\r\n",
                              (unsigned long)(ts_ms / 1000u),
                              (unsigned long)(ts_ms % 1000u),
                              ax_m, ay_m, az_m,
                              gx_m, gy_m, gz_m);
            HAL_UART_Transmit(&huart1, (uint8_t*)rawline, (uint16_t)rn, HAL_MAX_DELAY);

            next_print_ms += RAW_PRINT_PERIOD_MS;   /* schedule next print */
        }

        /* Sleep ~10ms to control loop rate */
        HAL_Delay(STEP_MS);
        ts_ms += 16;   /* track elapsed time in ms */

        /* ---- Debug: print sensor_data when set ---- */
        if (sensor_data != 0) {
            char sdline[24];
            int  sdn = snprintf(sdline, sizeof(sdline),
