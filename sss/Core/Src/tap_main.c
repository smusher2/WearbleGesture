...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	    static uint32_t ts_ms = 0;   // increases by ~10 ms per loop (matches HAL_Delay(10))
	  	  	  if (g_tap_irq)
	  	  	  {
	  	  		  g_tap_irq = 0;
	  	    ...
	  	    snprintf(msg1, sizeof(msg1),
	  	             "[%lu.%03lu] AX: %.3f, AY: %.3f, AZ: %.3f, GX: %.3f, GY: %.3f, GZ: %.3f \r\n",
	  	             (unsigned long)(ts_ms / 1000), (unsigned long)(ts_ms % 1000),
